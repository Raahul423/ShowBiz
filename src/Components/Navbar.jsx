...
const Navbar = () => {
  ...
  return (
    <div className='top-0 left-0  flex justify-around items-center p-6  w-full fixed z-50   '>
      <Link to={'/'}>
        <img src={assets.logo} alt='Error' className='w-44 bg-blend-color-burn bg-transparent cursor-pointer'  ></img>
      </Link>


      <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-large z-50 flex-col md:flex-row items-center md:justify-center gap-8 min-md:px-8 py-3 max-md:py-24 max-md:h-screen min-md:rounded-full backdrop-blur bg-black/70  md:bg-white/10  overflow-hidden transition-[width] duration-300 md:border border-gray-300/20  cursor-pointer  flex ${isopen ? 'max-sm:w-full':'max-sm:w-0'}`}>
